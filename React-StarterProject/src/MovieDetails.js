...
class MovieDetails extends Component {
    ...
    componentDidMount() {
      ...
      const searchItemsRef = firebase.database().ref('title_basics/'+this.props.match.params.id);
    ...
    ratingsRefQuery.orderByChild("tconst").endAt(this.props.match.params.tconst).once("value", (snapshot) => {
      ...
      this.setState({
          ...
          the same name in the searchResult variable
          */
      });
    });
    ...
    axios.get(`https://api.themoviedb.org/3/find/${this.props.match.params.tconst}`, {
      params: {
    ...
    .then(res => {
      ...
      if(this.state.artWorkObject && this.state.artWorkObject.length > 0){
        this.setState({
          poster: artWorkBaseURL_200+this.state.artWorkObject[0].poster_path,
          backdrop: artWorkBaseURL_1280+this.state.artWorkObject[0].backdrop_path,
          overview: this.state.artWorkObject[0].overview, 
          artWorkId: this.state.artWorkObject[0].id, 
        });
      }
    }).then(res => {
      axios.get(`https://api.themoviedb.org/3/movie/${this.state.artWorkId}/credits`, {
        params: {
